...
if (getgenv().BDD_V2_LOADED) then return; end

getgenv().BDD_V2_LOADED = true
...
local getnamecallmethod = getnamecall
local game = game
local table = table
...
if checkcaller and hookfunction and hookmetamethod and getnamecallmethod then
            ...
            if Instances ~= nil then
                if typeof(Instances[1]) == "Instance" and (table.find(Instances, CoreGui) or table.find(Instances, game)) then
                    if Instances[1] == CoreGui then
                        ...
                        if identifyexecutor and not identifyexecutor():find("Synapse") then setfenv(1, {}) end   
...
task.spawn(function()
    ...
    function PromptSaveInstance(inst)
        ...
        CurrentSaveInstanceWindow.MainWindow.Cancel.Up:connect(function()
            if CurrentSaveInstanceWindow then
                CurrentSaveInstanceWindow:Destroy()
                CurrentSaveInstanceWindow = nil
                if explorerPanel.Parent:FindFirstChild("SaveInstanceOverwriteCaution") then
...
BetterDarkDex.Name = ""
task.spawn(function()
pcall(function() if not gethui and syn and syn.protect_gui then syn.protect_gui(BetterDarkDex) end end)
pcall(function() if protect_gui then protect_gui(BetterDarkDex) end end)
end)
for Counter = 1, 20 do
